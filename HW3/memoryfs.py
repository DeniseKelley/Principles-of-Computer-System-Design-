...
#Creates a "hard" link 
  ...
  def Link(self, target, name, cwd):
    #"hard" link in the context of current working directory with inode number "cwd".
    # "name" is a string describing the name of the link, and "target" is a string describing 
    #the path to the target.

    #hard link is pretty much the same file with a different name
    #- it has same file size
    #- it has the same inode number 
    #- when original file is deleted there is no affect on the hard link file

    
    # b)ERROR_LINK_NOT_DIRECTORY
    ...
    # a)ERROR_LINK_TARGET_DOESNOT_EXIST
    # target does not exist in the file system
    ...
    file = InodeNumber(self.RawBlocks, target_inode)
    file.InodeNumberToInode()


    #d) ERROR_LINK_ALREADY_EXISTS
    # There is already a binding for "name" in the cwd




    #e)ERROR_LINK_TARGET_NOT_FILE
    # The target must be a file (INODE_TYPE_FILE)
    # if file.inode.type != INODE_TYPE_FILE:
    # e) The target must be a file (INODE_TYPE_FILE)
    if file.inode.type == INODE_TYPE_DIR:
      logging.debug ("ERROR_LINK_TARGET_NOT_FILE " + str(target))
      return -1, "ERROR_LINK_TARGET_NOT_FILE"

    #c) ERROR_LINK_DATA_BLOCK_NOT_AVAILABLE
    # There is not enough room left in the data blocks of the cwd to add another 
    # binding for this link)
    # Find available slot in directory data block
    ...
    link_inode = InodeNumber(self.RawBlocks, target_inode)
    link_inode.InodeNumberToInode()
    
    #link_inode.inode.type = INODE_TYPE_FILE
    #link_inode.inode.size = 0
    link_inode.inode.refcnt += 1
      # New files are not allocated any blocks; these are allocated on a Write()
    link_inode.StoreInode()

      # Add to parent's (filename,inode) table
    self.InsertFilenameInodeNumber(dir_inode, name, target_inode) 
...
Hint: link in HW#2, the solution to this problem does not involve a large amount of code. 
The keys to solve this problem are: 
1) make sure you clearly understand how data is stored in RawBlocks (see HW#1) and how to 
copy data from/to the RawBlocks class (which represents the actual disk storage in a real 
file system implementation) to the higher-level Python classes (such as InodeNumber() and 
Inode(); these represent what a kernel would work with in main memory in a real file system
 implementation); 
2) work on one sub-problem from the a) to g) list above at a time, 
 and make sure you test each sub-problem before moving to the next; 
3) test iteratively: 
 the showinode, showblock and showblockslice in the shell will help you debug and ensure 
 your implementation is doing what you intended it to do and that you are storing data in 
 the right format/place; 4) complement the existing debugging information with your own print 
 statements while you develop (comment these out before submission) to help in the process
...
#Symlink(self, target, name, cwd):
  ...
  def Symlink(self, target, name, cwd):
    ...
    #f) ERROR_SYMLINK_TARGET_EXCEEDS_BLOCK_SIZE
    ...
    #print("inode position ", inode_position)
    symlink_inode = InodeNumber(self.RawBlocks, inode_position)
    #print("symlink_inode ", symlink_inode)
    symlink_inode.InodeNumberToInode()
    symlink_inode.inode.type = INODE_TYPE_SYM
    symlink_inode.inode.size = 0
    symlink_inode.inode.refcnt = 1


    #?????????do i need to store two times
    symlink_inode.StoreInode()

    #print("target ", target )

    #symlink_inode.inode.block_numbers[0] = target
    stringbyte = bytearray(target,"utf-8")
    #print("stringbyte ", stringbyte)
    #block[0:len(target)] = stringbyte 
    #symlink_inode.inode.block_numbers[0] = stringbyte

    #_______________________________________________________________________

    # New files are not allocated any blocks; these are allocated on a Write()

   #symlink_inode.StoreInode() #CAN DELETE LATER

    

    """__________________________________________________
    
    Need to alocate a new block to store the link path
       __________________________________________________
    """
    new_block = self.AllocateDataBlock()
    # update inode (it will be written to raw storage before the method returns)
    ...
    symlink_inode.inode.block_numbers[0] = new_block
    ...
    # put path of the target into the block
    block[0:len(target)] = stringbyte 
...
Hint: link in HW#2, the solution to this problem does not involve a large amount of code. 
The keys to solve this problem are: 1) make sure you clearly understand how data is stored 
in RawBlocks (see HW#1) and how to copy data from/to the RawBlocks class (which represents 
the actual disk storage in a real file system implementation) to the higher-level Python 
classes (such as InodeNumber() and Inode(); these represent what a kernel would work with 
in main memory in a real file system implementation); 
2) work on one sub-problem from the a) to g) 
list above at a time, and make sure you test each sub-problem before moving to the next; 
3) test iteratively: the showinode, showblock and showblockslice in the shell will help 
you debug and ensure your implementation is doing what you intended it to do and that you 
are storing data in the right format/place; 4) complement the existing debugging information 
with your own print statements while you develop (comment these out before submission) 
