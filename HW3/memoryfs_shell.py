  ...
  def ls(self):
    ...
    while block_index <= (inobj.inode.size // BLOCK_SIZE):
      ...
      while current_position < end_position:
        ...
        #print("name and inode: ", entryname.decode(), entryinodenumber, " type: ", inobj2.inode.type)
        ...
        if inobj2.inode.type == INODE_TYPE_FILE:
          print ("[" + str(inobj2.inode.refcnt) + "]:" + entryname.decode())
            
          #print("entryinodenumber ", entryinodenumber)
        if inobj2.inode.type == INODE_TYPE_SYM:

          block_number = inobj2.inode.block_numbers[0]
          # first, we read the whole block from raw storage
          b = inobj2.RawBlocks.Get(block_number)
          print("[" + str(inobj2.inode.refcnt) + "]:" + entryname.decode() + "@ -> " + b.decode())
...
#    logging.info('Block (strings in block) [' + str(n) + '] : \n' + str((wholeblock[start:end+1].decode(encoding='UTF-8',errors='ignore'))))
  ...
  def lnh(self, target, name):
    ...
    """
    i = self.FileObject.GeneralPathToInodeNumber(target,self.cwd)
    print("lnh i gen path ", i)
    if i == -1:
      print ("Error: not found\n")
      return -1
    """
    ...
    written, errorcode = self.FileObject.Link(target, name, self.cwd)
    if written == -1:
      print ("Error: " + errorcode + "\n")
      return -1
    return 0
  ...
  def lns(self, target, name):
    ...
    written , errorcode = self.FileObject.Symlink(target, name, self.cwd)
    if written == -1:
      print ("Error: " + errorcode + "\n")
      return -1
    return 0
  ...
  def Interpreter(self):
    while (True):
      ...
      #lnh target name
        ...
        if len(splitcmd) != 3:
          print ("Error: link requires two arguments")
        else:
